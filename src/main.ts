...
import { registerRoute, initRouter, navigate, setDefaultRoute } from './router.ts'
import { renderSidebar, openMobileSidebar } from './components/sidebar.ts'
...
import { api, checkBackendHealth, isBackendOnline, isTauriRuntime, onBackendStatusChange } from './lib/tauri-api.ts'
const APP_VERSION = typeof __APP_VERSION__ !== 'undefined' ? __APP_VERSION__ : '0.0.0'
import { statusIcon } from './lib/icons.ts'
import { isForeignGatewayError, showGatewayConflictGuidance } from './lib/gateway-ownership.ts'
import { toast } from './components/toast.ts'
import { initI18n, t } from './lib/i18n.ts'
import { initFeatureGates } from './lib/feature-gates.ts'
import { onKernelChange } from './lib/kernel.ts'
import { showFloorBlocker, hideFloorBlocker } from './components/floor-blocker.ts'
import { registerEngine, initEngineManager, getActiveEngine, getActiveEngineId, needsInitialEngineChoice, isEngineSetupDeferred, adoptActiveEngineSelection, onEngineChange } from './lib/engine-manager.ts'
import openclawEngine from './engines/openclaw/index.ts'
import hermesEngine from './engines/hermes/index.ts'
import xintianEngine from './engines/xintian/index.ts'
import codexEngine from './engines/codex/index.ts'
import codewhaleEngine from './engines/codewhale/index.ts'
...
import './engines/codewhale/style/codewhale.css'
  ...
  overlay.innerHTML = `
    <div class="login-card" style="text-align:center">
      ...
      <div style="margin-top:16px;font-size:11px;color:#aaa">
        <a href="https://ai.iazp.cn" target="_blank" rel="noopener" style="color:#aaa;text-decoration:none">ai.iazp.cn</a>
...
function showLoginOverlay(defaultPw) {
  ...
  overlay.innerHTML = `
    <div class="login-card">
      ${_logoSvg}
      <div class="login-title">智爪平台</div>
...
async function boot() {
  ...
  registerEngine(codewhaleEngine)
  registerEngine(xintianEngine)
  registerRoute('/engine-select', () => import('./pages/engine-select.ts'))
  ...
  topbar.innerHTML = `
    ...
    <span class="mobile-topbar-title">智爪平台</span>
  `
  ...
  setTimeout(async () => {
    try {
      const { checkAndWarnCiaoBug } = await import('./lib/ciao-bug-warning.ts')
      checkAndWarnCiaoBug()
    } catch (err) {
    ...
    if (isTauriRuntime() && getActiveEngineId() === 'openclaw') {
      import('@tauri-apps/api/event').then(async ({ listen }) => {
        const refreshAfterTask = async () => {
          // 清除 API 缓存，确保拿到最新状态
          const { invalidate } = await import('./lib/tauri-api.ts')
    ...
    if (foreign) {
      ...
      banner.querySelector('#btn-gw-claim')?.addEventListener('click', async (e) => {
        ...
        try {
          ...
          // 认领后立刻刷新全局状态
          const { refreshGatewayStatus } = await import('./lib/app-state.ts')
  ...
  try {
    ...
    banner.innerHTML = `
      <div class="update-banner-content">
        ...
        <a class="btn btn-sm" href="https://ai.iazp.cn" target="_blank" rel="noopener">${t('about.downloadFromWebsite')}</a>
...
;(async () => {
  ...
  try {
    ...
    if (app) app.innerHTML = `
      <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:100vh;padding:20px;text-align:center;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif">
        ...
        <div style="margin-top:24px;font-size:11px;color:#a1a1aa">${t('common.pageLoadFailedHint')}<br><a href="https://ai.iazp.cn" target="_blank" style="color:#6366f1">${t('about.linkWebsite')}</a></div>
      </div>`
  }
  startUpdateChecker()
  ...
  setTimeout(async () => {
    const { initAIFab, registerPageContext, openAIDrawerWithError } = await import('./components/ai-drawer.ts')
    initAIFab()

    // 注册各页面上下文提供器
    registerPageContext('/chat-debug', async () => {
      const { isOpenclawReady, isGatewayRunning } = await import('./lib/app-state.ts')
    ...
    registerPageContext('/services', async () => {
      const { isGatewayRunning } = await import('./lib/app-state.ts')
      const { api } = await import('./lib/tauri-api.ts')
      const lines = ['## 服务状态']
      lines.push(`- Gateway: ${isGatewayRunning() ? '运行中' : '未运行'}`)
      try {
