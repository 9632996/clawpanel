...
import { api, safeTauriListen } from '../../../lib/tauri-api.ts'
import {
  loadHermesProviders,
...
export function render() {
  ...
  let nativeDashboard = { running: false, port: 9119 }
  ...
  function draw() {
    ...
    el.innerHTML = `
      ...
      <div class="hm-native-dashboard-hint">
        <div class="hm-native-dashboard-status">
          <span class="hm-dot hm-dot--${nativeDashboard?.running ? 'run' : 'stop'}"></span>
          <span>${t('engine.dashNativePanel')} · 127.0.0.1:${nativeDashboard?.port || 9119}</span>
          <strong>${nativeDashboard?.running ? t('engine.dashRunning') : t('engine.dashStopped')}</strong>
        </div>
        <div class="hm-native-dashboard-actions">
          <button class="hm-btn hm-btn--sm hm-dash-native-probe" ${nativeBusy ? 'disabled' : ''}>${t('engine.dashRefresh')}</button>
          ${!nativeDashboard?.running ? `<button class="hm-btn hm-btn--primary hm-btn--sm hm-dash-native-start" ${nativeBusy ? 'disabled' : ''}>${nativeBusy ? t('engine.dashNativePanelStarting') : t('engine.dashNativePanelStart')}</button>` : ''}
          ${nativeDashboard?.running ? `<button class="hm-btn hm-btn--danger hm-btn--sm hm-dash-native-stop" ${nativeBusy ? 'disabled' : ''}>${t('engine.dashNativePanelStop')}</button>` : ''}
          <button class="hm-btn hm-btn--sm hm-dash-open-native" data-href="${HERMES_DASHBOARD_URL}" ${nativeBusy ? 'disabled' : ''}>
  ...
  function bind() {
    ...
    const openNativeDashboardUrl = async (port = 9119) => {
      const url = HERMES_DASHBOARD_URL.replace(/:9119(\/?$)/, ':' + port + '$1')
      await openExternalUrl(url)
    }
    const updateNativeProbe = async () => {
      nativeDashboard = await api.hermesDashboardProbe().catch(() => ({ running: false, port: nativeDashboard?.port || 9119 }))
      return nativeDashboard
    }
    el.querySelector('.hm-dash-native-probe')?.addEventListener('click', async () => {
      nativeBusy = true; draw()
      await updateNativeProbe()
      nativeBusy = false; draw()
    })
    el.querySelector('.hm-dash-native-start')?.addEventListener('click', async () => {
      nativeBusy = true; draw()
      try {
        const result = await api.hermesDashboardStart()
        nativeDashboard = { running: !!result?.started, port: result?.port || 9119 }
        if (!result?.started) {
          const { toast } = await import('../../../components/toast.ts')
          const detail = (result?.log_tail || '').split('\n').slice(-3).join('\n').trim()
          toast(t('engine.dashNativePanelStartGeneric') + (detail ? ': ' + detail : ''), 'error', { duration: 8000 })
        }
      } catch (err) {
        const { toast } = await import('../../../components/toast.ts')
        toast(t('engine.dashNativePanelStartGeneric') + ': ' + (err?.message || err), 'error')
      } finally {
        nativeBusy = false; await refresh()
      }
    })
    el.querySelector('.hm-dash-native-stop')?.addEventListener('click', async () => {
      nativeBusy = true; draw()
      try { await api.hermesDashboardStop() }
      catch (err) {
        const { toast } = await import('../../../components/toast.ts')
        toast(String(err?.message || err).replace(/^Error:\s*/, ''), 'error')
      } finally {
        nativeDashboard = { running: false, port: nativeDashboard?.port || 9119 }
        nativeBusy = false; await refresh()
      }
    })
    ...
    el.querySelector('.hm-dash-open-native')?.addEventListener('click', async (e) => {
      ...
      const tryOpen = async (port) => openNativeDashboardUrl(port)
      ...
      const startAndOpen = async () => {
        ...
        try {
          ...
          if (result?.port) nativeDashboard = { running: !!result?.started, port: result.port }
          if (result?.started) {
            try {
      ...
      try {
        ...
        const { showContentModal, showUpgradeModal } = await import('../../../components/modal.ts')
    ...
    try {
      ...
      if (wasRunning) {
        try {
          ...
          await api.hermesGatewayAction('start')
          cfgMsg = `<span style="color:var(--success)">✓ ${t('engine.configSavedGatewayRestarted')}</span>`
        } catch (restartErr) {
          cfgMsg = `<span style="color:var(--warning)">✓ ${t('engine.configSavedGatewayRestartFailed')}: ${String(restartErr).replace(/^Error:\s*/, '')}</span>`
        }
      } else {
  ...
  async function refresh() {
    try {
      ...
      try { hermesConfig = await api.hermesReadConfig() } catch (_) {}
      try { nativeDashboard = await api.hermesDashboardProbe() } catch (_) { nativeDashboard = { running: false, port: nativeDashboard?.port || 9119 } }
    } catch (_) {}
    loading = false
