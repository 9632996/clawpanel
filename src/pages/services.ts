 ...
 * 服务启停 + 更新检测 + 配置备份管理
 */
import { api, isTauriRuntime } from '../lib/tauri-api.ts'
import { toast } from '../components/toast.ts'
import { humanizeError } from '../lib/humanize-error.ts'
import { showConfirm, showModal, showUpgradeModal } from '../components/modal.ts'
import { isGatewayOperationActive, isGatewayRunning, isMacPlatform, isInDocker, markGatewayRunning, markGatewayStopped, onGatewayChange, onGatewayOperationChange, runGatewayOperation, setUpgrading, setUserStopped, resetAutoRestart, syncGatewayActionButtons, waitForGatewayServiceState, writeGatewayConfig } from '../lib/app-state.ts'
import { isForeignGatewayError, isForeignGatewayService, maybeShowForeignGatewayBindingPrompt, showGatewayConflictGuidance } from '../lib/gateway-ownership.ts'
import { diagnoseInstallError } from '../lib/error-diagnosis.ts'
...
async function loadVersion(page) {
  ...
  try {
    ...
    const dockerImage = (panelConfig?.dockerDefaultImage || '').trim() || '1186258278/openclaw'
...
function configuredDockerImage(panelConfig) {
  return (panelConfig?.dockerDefaultImage || '').trim() || '1186258278/openclaw'
}
...
async function handleSaveConfig(page, restart) {
  ...
  try {
    if (restart && isGatewayRunning()) {
      await writeGatewayConfig(config, {
        apply: 'now',
        reason: 'services-config-save-now',
        label: t('services.actionProgress', { action: t('services.restart') }),
      })
    } else {
      await writeGatewayConfig(config, { apply: false, reason: 'services-config-save' })
    }
    ...
    if (restart) {
      if (isGatewayRunning()) toast(t('services.gwRestarted'), 'success')
      else toast(`${t('services.configSaved')}, ${t('settings.effectNextLaunch')}`, 'success')
      await loadServices(page)
    }
