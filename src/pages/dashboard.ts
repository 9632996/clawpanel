...
import { getActiveInstance, markGatewayRunning, markGatewayStopped, onGatewayChange, onGatewayOperationChange, resetAutoRestart, runGatewayOperation, setUserStopped, syncGatewayActionButtons, waitForGatewayServiceState, writeGatewayConfig } from '../lib/app-state.ts'
import { isForeignGatewayError, isForeignGatewayService, maybeShowForeignGatewayBindingPrompt, showGatewayConflictGuidance, showInstallationCleanup } from '../lib/gateway-ownership.ts'
import { navigate } from '../router.ts'
import { t } from '../lib/i18n.ts'
import { wsClient } from '../lib/ws-client.ts'
import { attachCliConflictBanner } from '../components/cli-conflict-banner.ts'
  ...
  if (config) {
    ...
    if (needsPatch) {
      try {
        ...
        if (patched) {
          config = freshConfig
          writeGatewayConfig(freshConfig, { apply: false, reason: 'dashboard-self-heal' }).catch(() => {})
        }
      } catch {}
    }
  }
