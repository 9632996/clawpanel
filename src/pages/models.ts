...
import { humanizeError } from '../lib/humanize-error.ts'
import { showModal, showConfirm } from '../components/modal.ts'
import { icon, statusIcon } from '../lib/icons.ts'
import { API_TYPES, PROVIDER_PRESETS, QTCOOL, MODEL_PRESETS, fetchQtcoolModels } from '../lib/model-presets.ts'
import { t } from '../lib/i18n.ts'
import { applyPendingGatewayConfig, cancelGatewayConfigApply, scheduleGatewayConfigApply, writeGatewayConfig } from '../lib/app-state.ts'
import { termHelpHtml, attachTermTooltips } from '../lib/term-tooltip.ts'
...
async function loadConfig(page, state) {
  ...
  try {
    ...
    if (before !== after || normalizedModel.changed) {
      ...
      await writeGatewayConfig(state.config, { apply: false, reason: 'models-self-heal' })
    ...
    return `
      <div class="model-card" data-model-id="${id}" data-full="${full}"
        ...
        <div style="display:flex;gap:6px;flex:0 1 auto;flex-wrap:wrap;justify-content:flex-end;margin-left:auto">
          ...
          <button class="btn btn-sm btn-secondary" data-action="apply-tool">应用到工具</button>
...
async function saveConfigOnly(state) {
  try {
    ...
    normalizeProviderUrls(state.config)
    await writeGatewayConfig(state.config, { apply: false, reason: 'models-metadata' })
  } catch (e) {
...
async function doAutoSave(state) {
  try {
    ...
    if (primary) applyDefaultModel(state)
    normalizeProviderUrls(state.config)
    await writeGatewayConfig(state.config, { apply: false, reason: 'models-page' })
    ...
    const gwRunning = gw?.running === true && gw?.owned_by_current_instance !== false
    if (gwRunning) {
      // 配置已写入。使用 3s 防抖 + 单飞行锁排队重启，避免快速连续编辑触发多次重启。
      showConfigApplyToast()
      scheduleGatewayConfigApply({ reason: 'models-page' })
    } else {
...
function showConfigApplyToast() {
  ...
  applyNow.onclick = async () => {
    try {
      const result = await applyPendingGatewayConfig({ force: true, label: t('models.restarting') })
      if (result.applied) toast(t('models.configEffective'), 'success')
      else if (result.reason === 'gateway-stopped') toast(t('models.configSavedGwNotRunning'), 'info')
    } catch (err) {
      const retryBtn = document.createElement('button')
      retryBtn.className = 'btn btn-sm btn-primary'
      retryBtn.textContent = t('models.retryRestart')
      retryBtn.style.marginLeft = '8px'
      retryBtn.onclick = () => scheduleGatewayConfigApply({ delayMs: 0, reason: 'models-retry' })
      toast(t('models.configSavedGwFailed') + ': ' + (err?.message || String(err)), 'warning', { action: retryBtn, duration: 6000 })
    }
  }
...
async function handleAction(action, btn, card, section, providerKey, provider, page, state) {
  switch (action) {
    ...
    case 'apply-tool': {
      if (!card) return
      applyModelToTools(page, state, providerKey, provider, card.dataset.modelId)
      break
    }
  }
}

async function applyModelToTools(page, state, providerKey, provider, modelId) {
  let tools = []
  try {
    tools = await api.listModelTools()
  } catch (e) {
    toast(`读取工具模板失败: ${e?.message || e}`, 'error')
    return
  }
  const defaultTools = new Set(['openclaw', 'codex', 'codewhale', 'hermes'])
  const overlay = document.createElement('div')
  overlay.className = 'modal-overlay'
  overlay.innerHTML = `
    <div class="modal" style="max-width:560px">
      <div class="modal-title">应用模型到工具</div>
      <div style="font-size:13px;color:var(--text-secondary);line-height:1.6;margin-bottom:12px">
        <div>模型：<code>${escapeHtml(providerKey)}/${escapeHtml(modelId)}</code></div>
        <div>端点：<code>${escapeHtml(provider.baseUrl || '')}</code></div>
      </div>
      <div style="display:grid;gap:8px;margin-bottom:14px">
        ${tools.map(tool => `
          <label style="display:flex;align-items:flex-start;gap:10px;padding:10px 12px;border:1px solid var(--border-primary);border-radius:var(--radius-md);background:var(--bg-secondary);cursor:pointer">
            <input type="checkbox" class="model-tool-check" value="${escapeHtml(tool.id)}" ${defaultTools.has(tool.id) ? 'checked' : ''} style="margin-top:3px">
            <span style="display:grid;gap:2px">
              <strong style="color:var(--text-primary);font-size:13px">${escapeHtml(tool.name || tool.id)}</strong>
              <span style="color:var(--text-tertiary);font-size:12px">${escapeHtml((tool.configFiles || []).join('、'))}</span>
            </span>
          </label>
        `).join('')}
      </div>
      <div class="modal-actions">
        ...
        <button class="btn btn-primary btn-sm" data-action="confirm">应用</button>
      </div>
    </div>
  `
  ...
  overlay.querySelector('[data-action="confirm"]').onclick = async () => {
    const selected = [...overlay.querySelectorAll('.model-tool-check:checked')].map(el => el.value)
    if (!selected.length) { toast('请选择至少一个工具', 'warning'); return }
    ...
    const modelInfo = {
      ...
      apiKey: apiKeyDisplayValue(provider.apiKey || ''),
      model: modelId,
    ...
    for (const toolId of selected) {
      try {
        ...
        failed.push(`${toolId}: ${e?.message || e}`)
      }
    }
    overlay.remove()
    if (ok.includes('openclaw')) {
      try {
        state.config = await api.readOpenclawConfig()
        renderDefaultBar(page, state)
        renderProviders(page, state)
    ...
    if (failed.length) {
      toast(`已应用 ${ok.length} 个，失败 ${failed.length} 个：${failed.join('；')}`, 'warning', { duration: 8000 })
    } else {
      toast(`已应用到 ${ok.length} 个工具`, 'success')
    }
  }
}
