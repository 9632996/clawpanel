 ...
 * 一键检测 + 一键修复，简洁易懂
 */
import { api, getRequestLogs, clearRequestLogs, isTauriRuntime } from '../lib/tauri-api.ts'
import { wsClient } from '../lib/ws-client.ts'
import { isOpenclawReady, isGatewayRunning, resetAutoRestart, runGatewayOperation, setUserStopped } from '../lib/app-state.ts'
...
async function testWebSocketAdv(page) {
  ...
  try {
    ...
    const isLocalHost = /^(127\.0\.0\.1|localhost)(:\d+)?$/i.test(wsHost)
    const wsScheme = (!isLocalHost && location.protocol === 'https:') ? 'wss' : 'ws'
