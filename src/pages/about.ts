...
import { showUpgradeModal, showConfirm, showContentModal } from '../components/modal.ts'
import { setUpgrading } from '../lib/app-state.ts'
import { icon, statusIcon } from '../lib/icons.ts'
import { t, getLang } from '../lib/i18n.ts'
import { getActiveEngineId } from '../lib/engine-manager.ts'
  ...
  page.innerHTML = `
    <div class="page-header" style="display:flex;align-items:center;gap:16px">
      ...
      <div>
        ...
        <p class="page-desc" style="margin:0">${t('about.subtitle')} · <a href="https://ai.iazp.cn" target="_blank" rel="noopener" style="color:var(--primary)">ai.iazp.cn</a></p>
      </div>
    </div>
  ...
  try {
    if (window.__TAURI_INTERNALS__) {
      ...
      unlistenError = await listen('upgrade-error', async (e) => {
        ...
        const { diagnoseInstallError } = await import('../lib/error-diagnosis.ts')
  ...
  try {
    ...
    if (isFakeUpdate) {
      meta.innerHTML = `
        ...
        <a class="btn btn-primary btn-sm" href="https://ai.iazp.cn" target="_blank" rel="noopener" style="${btnSm}">${t('about.downloadFullInstaller')}</a>
      `
      ...
      meta.innerHTML = `
        ...
        <a class="btn btn-primary btn-sm" href="https://ai.iazp.cn" target="_blank" rel="noopener" style="${btnSm}">${t('about.downloadFromWebsite')}</a>
      `
    } else {
    ...
    if (!meta) return
    if (isFakeUpdate) {
      meta.innerHTML = `<span style="color:var(--warning)">⚠️ ${t('about.versionMismatch', { frontend: panelVersion, binary: binaryVersion })}</span> <a class="btn btn-primary btn-sm" href="https://ai.iazp.cn" target="_blank" rel="noopener" style="${btnSm}">${t('about.downloadFullInstaller')}</a>`
    } else {
      meta.innerHTML = `<span style="color:var(--text-tertiary)">${t('about.checkUpdateFailed')}</span> <a class="btn btn-secondary btn-sm" href="https://ai.iazp.cn" target="_blank" rel="noopener" style="${btnSm}">${t('about.goToWebsite')}</a>`
    }
  }
}
...
const LINKS = [
  { label: t('about.linkWebsite'), url: 'https://ai.iazp.cn', primary: true },
...
function renderContribute(page) {
  ...
  el.innerHTML = `
    ...
    <div style="display:flex;flex-wrap:wrap;gap:8px">
      <a class="btn btn-primary btn-sm" href="https://ai.iazp.cn" target="_blank" rel="noopener">${t('about.linkWebsite')}</a>
    </div>
    ...
    <div style="display:flex;flex-direction:column;gap:12px">
      ...
      <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:12px;font-size:var(--font-size-sm)">
        ...
        <div style="padding:12px;border-radius:var(--radius-md);border:1px solid var(--border-primary);background:var(--bg-secondary)">
          ...
          <a href="https://ai.iazp.cn" target="_blank" rel="noopener" style="color:var(--accent)">ai.iazp.cn</a>
        </div>
        <div style="padding:12px;border-radius:var(--radius-md);border:1px solid var(--border-primary);background:var(--bg-secondary)">
        ...
        <div style="padding:12px;border-radius:var(--radius-md);border:1px solid var(--border-primary);background:var(--bg-secondary)">
          ...
          <a href="https://ai.iazp.cn" target="_blank" rel="noopener" style="color:var(--accent)">${t('about.contactViaWebsite')}</a>
        </div>
      </div>
