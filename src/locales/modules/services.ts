...
export default {
  ...
  uninstallImpactStop: _('Gateway 将立即停止，所有连接到当前工作台的渠道会下线', 'Gateway will stop immediately and all channels connected to the workbench will go offline', 'Gateway 將立即停止，所有連接到目前工作台的頻道會下線', 'Gateway は直ちに停止し、現在のワークベンチに接続中のチャンネルがオフラインになります', 'Gateway가 즉시 중지되고 현재 워크벤치에 연결된 모든 채널이 오프라인 상태가 됩니다', 'Gateway sẽ dừng ngay lập tức và tất cả kênh kết nối tới workbench hiện tại sẽ ngoại tuyến', 'El Gateway se detendrá de inmediato y todos los canales conectados al workbench quedarán desconectados', 'O Gateway será interrompido imediatamente e todos os canais conectados ao workbench ficarão offline', 'Gateway будет немедленно остановлен, и все каналы, подключённые к текущей рабочей среде, перейдут в офлайн', 'Le Gateway s’arrêtera immédiatement et tous les canaux liés à l’atelier passeront hors ligne', 'Gateway wird sofort gestoppt und alle mit der Workbench verbundenen Kanäle gehen offline'),
  ...
  upgradeLatestConfirm: _('确定要将 OpenClaw 升级到最新上游{source}{version}吗？\n最新版可能包含尚未完全验证的接口或配置变化，升级过程中 Gateway 会短暂中断。', 'Upgrade OpenClaw to the latest upstream {source}{version}?\nThe latest version may include API or config changes that are not fully verified yet. Gateway will be briefly interrupted.', '確定要將 OpenClaw 升級到最新上游{source}{version}嗎？\n最新版可能包含尚未完全驗證的介面或設定變化，升級過程中 Gateway 會短暫中斷。'),
  ...
  protocolBadgeTitle: _('当前 Gateway 与工作台协商出的 WebSocket 握手协议版本（与设备签名 payload 的 v3 格式无关）', 'WebSocket handshake protocol version negotiated between Gateway and the workbench (independent of the device signature payload v3 format)', '目前 Gateway 與工作台協商出的 WebSocket 握手協議版本（與設備簽名 payload 的 v3 格式無關）'),
}
