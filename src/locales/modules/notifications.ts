...
export default {
  ...
  desc: _(
    '即使工作台关闭，Windows / macOS / iOS 通知中心也能收到 Agent / Cron / 渠道消息',
    'Receive Agent / Cron / channel messages in Windows / macOS / iOS notification center even when the workbench is closed',
    '即使工作台關閉，Windows / macOS / iOS 通知中心也能收到 Agent / Cron / 頻道訊息',
    'ワークベンチを閉じても、Windows / macOS / iOS の通知センターで Agent / Cron / チャネルメッセージを受信',
    '워크벤치를 닫아도 Windows / macOS / iOS 알림 센터에서 Agent / Cron / 채널 메시지 수신',
    'Nhận tin nhắn Agent / Cron / kênh ngay cả khi workbench đã đóng',
    'Recibe mensajes de Agent / Cron / canal incluso con el workbench cerrado',
    'Receba mensagens de Agent / Cron / canal mesmo com o workbench fechado',
    'Получайте сообщения Agent / Cron / каналов даже когда рабочая среда закрыта',
    'Recevez les messages Agent / Cron / canal même lorsque l’atelier est fermé',
    'Empfange Agent / Cron / Kanal-Nachrichten auch bei geschlossener Workbench'
  ),
  ...
  hint: _(
    '点「启用」后浏览器会弹权限请求。授权后即使关闭工作台也能收通知。',
    'Click "Enable" — the browser will ask for permission. Once granted, notifications arrive even if the workbench is closed.',
    '點「啟用」後瀏覽器會彈權限請求。授權後即使關閉工作台也能收通知。',
    '「有効化」をクリックするとブラウザが権限を要求します。許可後はワークベンチを閉じていても通知が届きます。',
    '"활성화"를 클릭하면 브라우저가 권한을 요청합니다. 허용 후에는 워크벤치를 닫아도 알림을 받을 수 있습니다.',
    'Nhấn "Bật" — trình duyệt sẽ yêu cầu quyền. Sau khi cấp, thông báo vẫn đến khi workbench đã đóng.',
    'Haz clic en "Activar"; el navegador pedirá permiso. Una vez concedido, recibirás notificaciones aunque el workbench esté cerrado.',
    'Clique em "Ativar"; o navegador pedirá permissão. Após concedida, você recebe notificações mesmo com o workbench fechado.',
    'Нажмите «Включить» — браузер запросит разрешение. После согласия уведомления будут приходить, даже если рабочая среда закрыта.',
    'Cliquez sur « Activer » — le navigateur demandera la permission. Une fois accordée, les notifications arrivent même si l’atelier est fermé.',
    'Klicken Sie auf „Aktivieren" — der Browser fragt nach Berechtigung. Nach Erteilung kommen Benachrichtigungen auch bei geschlossener Workbench.'
  ),
  ...
  testTitle: _('智爪平台测试通知', 'Workbench Test', '智爪平台測試通知', 'ワークベンチのテスト通知', '워크벤치 테스트 알림', 'Thông báo thử Workbench', 'Prueba de Workbench', 'Teste da Workbench', 'Тест рабочей среды', 'Test de l’atelier', 'Workbench-Test'),
