...
export default {
  ...
  qtcoolName: _('AI作品模型服务', 'AI作品 Model Service', 'AI作品模型服務'),
  qtcoolRecommend: _('推荐', 'Recommended', '推薦'),
  qtcoolDesc: _('智爪工具入口内置的 OpenAI 兼容模型服务，正式客户版可使用构建器注入的授权额度。', 'OpenAI-compatible model service built into the Zhizhua tool portal. Customer builds can use authorized credits injected by the builder.', '智爪工具入口內置的 OpenAI 相容模型服務，正式客戶版可使用構建器注入的授權額度。', 'OpenAI 互換のモデルサービス', 'OpenAI 호환 모델 서비스'),
  qtcoolMore: _('了解更多 →', 'Learn more →'),
  qtcoolCheckin: _('打开智爪工具入口', 'Open Zhizhua Tool Portal', '開啟智爪工具入口'),
  qtcoolKeyPlaceholder: _('粘贴 API Key（正式版通常已自动注入）', 'Paste API Key (usually injected automatically in customer builds)', '貼上 API Key（正式版通常已自動注入）'),
  qtcoolFetchModels: _('获取模型列表', 'Fetch Model List', '取得模型列表'),
  qtcoolFetching: _('获取中...', 'Fetching...', '取得中...'),
  qtcoolNoKey: _('需要测试或补充额度？前往', 'Need test or additional credits? Go to', '需要測試或補充額度？前往'),
  qtcoolCheckinPage: _('智爪工具入口', 'Zhizhua tool portal', '智爪工具入口'),
  qtcoolCheckinHint: _('查看授权与模型服务状态，并在', 'to view authorization and model service status, then open', '查看授權與模型服務狀態，並在'),
  qtcoolDashboard: _('工具入口', 'tool portal', '工具入口'),
  qtcoolCopyKey: _('完成额度管理', 'for credit management', '完成額度管理'),
  qtcoolSelectTitle: _('选择要添加的模型', 'Select models to add', '選擇要新增的模型'),
  qtcoolSelectHint: _('从 AI作品模型服务获取到 {count} 个可用模型，勾选需要的模型后点击添加。', 'Fetched {count} available models from AI作品 Model Service. Select and add.', '從 AI作品模型服務取得到 {count} 個可用模型，勾選需要的模型后點擊新增。'),
  qtcoolKeyLabel: _('API Key', 'API Key'),
  qtcoolKeyCheckinLink: _('打开智爪工具入口 →', 'Open Zhizhua tool portal →', '開啟智爪工具入口 →'),
  qtcoolKeyPlaceholder2: _('粘贴你的 API Key', 'Paste your API Key', '貼上你的 API Key'),
  qtcoolNoKeyWarn: _('请输入 API Key（正式客户版通常会自动注入）', 'Please enter an API Key (usually injected automatically in customer builds)', '請輸入 API Key（正式客戶版通常會自動注入）'),
  ...
  qtcoolProviderAdded: _('已添加 AI作品模型服务（{count} 个模型）', 'Added AI作品 Model Service ({count} models)', '已新增 AI作品模型服務（{count} 個模型）'),
  already: _('已有', 'Exists'),
