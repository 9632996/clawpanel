...
export default {
  ...
  qtcoolDesc: _('智爪工具入口提供的 OpenAI 兼容模型服务，可使用内置授权或你自己的 API Key。', 'OpenAI-compatible model service from the Zhizhua tool portal. Use the built-in authorization or your own API key.', '智爪工具入口提供的 OpenAI 相容模型服務，可使用內置授權或你自己的 API Key。', 'OpenAI 互換のモデルサービス', 'OpenAI 호환 모델 서비스'),
  qtcoolMore: _('了解更多 →', 'Learn more →'),
  qtcoolCheckin: _('打开智爪工具入口', 'Open Zhizhua Tool Portal', '開啟智爪工具入口'),
  qtcoolKeyPlaceholder: _('粘贴你的 API Key（可选）', 'Paste your API Key (optional)', '貼上你的 API Key（可選）'),
  ...
  qtcoolNoKeyWarn: _('请输入 API Key，或先使用已内置的默认模型配置', 'Please enter an API key, or use the built-in default model configuration first', '請輸入 API Key，或先使用已內置的預設模型設定'),
