...
export default {
  ...
  headerTitle: _('欢迎使用智爪平台', 'Welcome to the AI Workbench', '', '智爪平台へようこそ', '지다오 AI 워크벤치에 오신 것을 환영합니다', 'Chào mừng đến với AI Workbench', 'Bienvenido al AI Workbench', 'Bem-vindo ao AI Workbench', 'Добро пожаловать в AI Workbench', 'Bienvenue dans AI Workbench', 'Willkommen bei der AI Workbench'),
  ...
  officialWebsite: _('智爪工具入口', 'Zhizhua Tool Portal', '智爪工具入口', 'ツールポータル', '도구 포털', 'Cổng công cụ', 'Portal de herramientas', 'Portal de ferramentas', 'Портал инструментов', 'Portail outils', 'Tool-Portal'),
  ...
  macNodeHint: _('macOS 上从 Finder 启动可能找不到 Node.js。试试关掉当前应用后从终端启动：', 'On macOS, launching from Finder may not find Node.js. Try closing the app and launching from terminal:', 'macOS 上從 Finder 啟動可能找不到 Node.js。試試關掉目前應用後從終端啟動：'),
  ...
  aiAssistant: _('智爪助手', 'AI Assistant', '智爪助手', 'AI アシスタント', 'AI 어시스턴트', 'Trợ lý AI', 'Asistente IA', 'Assistente IA', 'ИИ-ассистент', 'Assistant IA', 'KI-Assistent'),
  ...
  installHint: _('点击安装后，将默认安装当前产品版本绑定的推荐稳定版；如需升降级，可稍后到「关于」页面切换版本。', 'Installation will use the recommended stable version bound to the current product release. To change versions later, go to the About page.', '點擊安裝后，將預設安裝目前產品版本綁定的推薦穩定版；如需升降級，可稍后到「關於」頁面切換版本。'),
  ...
  methodHintR2: _('从智爪工具入口下载独立安装包，自带 Node.js 运行时，无需 npm。国内下载速度最快。', 'Downloads standalone package from the Zhizhua tool portal with bundled Node.js runtime, no npm needed. Fastest for China.', '從智爪工具入口下載獨立安裝包，自帶 Node.js 執行時，無需 npm。國內下載速度最快。'),
  ...
  envHintDesc: _('桌面版只能直接管理<strong>当前 Windows / macOS / Linux 本机</strong>里的 OpenClaw。若你的 OpenClaw 实际装在 WSL、Docker 或远程服务器里，请把下面内容视为<strong>高级替代方案</strong>，而不是普通本机安装的默认路径。', 'The desktop app can only directly manage OpenClaw installed on the <strong>current local Windows / macOS / Linux machine</strong>. If your OpenClaw actually lives in WSL, Docker or a remote server, treat the options below as <strong>advanced alternatives</strong>, not the default local setup path.', '桌面版只能直接管理<strong>目前 Windows / macOS / Linux 本機</strong>裡的 OpenClaw。若你的 OpenClaw 實際裝在 WSL、Docker 或遠端伺服器裡，請把下面內容視為<strong>進階替代方案</strong>，而不是普通本機安裝的預設路徑。'),
  ...
  envHintInstallManage: _('高级替代方案：在对应环境里使用 Web 版面板', 'Advanced alternative: use the Web panel inside that environment', '進階替代方案：在對應環境裡使用 Web 版面板'),
  ...
  dockerDefaultImagePlaceholder: _('例如 1186258278/openclaw', 'e.g. 1186258278/openclaw', '例如 1186258278/openclaw'),
  ...
  wslWebDesc: _('如果你的 OpenClaw 本来就运行在 WSL 里，可打开 WSL 终端部署 Web 版面板：', 'If your OpenClaw already runs inside WSL, open a WSL terminal and deploy the Web panel there:', '如果你的 OpenClaw 本來就運行在 WSL 裡，可開啟 WSL 終端部署 Web 版面板：'),
  ...
  dockerDesc: _('如果你的 OpenClaw 本来就运行在容器里，可在容器内安装 OpenClaw + Web 版面板：', 'If your OpenClaw already runs inside a container, install OpenClaw + the Web panel there:', '如果你的 OpenClaw 本來就運行在容器裡，可在容器內安裝 OpenClaw + Web 版面板：'),
