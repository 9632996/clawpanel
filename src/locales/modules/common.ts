...
export default {
  ...
  backendDownDesc: _('工作台后端服务未运行，无法获取真实数据。', 'The workbench backend is not running, so live data is unavailable.', '工作台後端服務未執行，無法取得真實資料。', 'ワークベンチのバックエンドが実行されていないため、実データを取得できません。', '워크벤치 백엔드가 실행 중이 아니어서 실제 데이터를 가져올 수 없습니다.', 'Dịch vụ backend của workbench chưa chạy nên không thể lấy dữ liệu thực.'),
