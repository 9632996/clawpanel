...
export default {
  ...
  desc: _('智爪多智能体工作台版本和项目信息', 'Product version and project info', '智爪多智能體工作台版本和項目資訊', '製品バージョンとプロジェクト情報', '제품 버전 및 프로젝트 정보', 'Thông tin phiên bản và dự án của sản phẩm', 'Información de versión y proyecto del producto', 'Informações de versão e projeto do produto', 'Информация о версии и проекте продукта', 'Informations de version et de projet du produit', 'Produktversions- und Projektinformationen'),
  subtitle: _('OpenClaw 多智能体工作台', 'OpenClaw Multi-Agent Workbench', '', 'OpenClaw マルチエージェントワークベンチ', 'OpenClaw 멀티 에이전트 워크벤치', 'Workbench đa tác nhân OpenClaw', 'Workbench multiagente de OpenClaw', 'Workbench multiagente do OpenClaw', 'Мультиагентная рабочая среда OpenClaw', 'Atelier multi-agent OpenClaw', 'OpenClaw Multi-Agent-Workbench'),
  ...
  techStack: _('智爪多智能体工作台基于 Tauri v2 构建，前端 Vanilla JS + Vite，后端 Rust。', 'Built with Tauri v2, Vanilla JS + Vite frontend, and Rust backend.', '智爪多智能體工作台基於 Tauri v2 構建，前端 Vanilla JS + Vite，後端 Rust。', 'Tauri v2、Vanilla JS + Vite フロントエンド、Rust バックエンドで構築されています。'),
  copyright: _('© 2026 AI作品 · 智爪多智能体工作台', '© 2026 AI作品 · Zhizhua AI Workbench', '© 2026 AI作品 · 智爪多智能體工作台'),
  ...
  versionAvailable: _('智爪多智能体工作台 v{version} 可用', 'Version v{version} available', '智爪多智能體工作台 v{version} 可用', 'v{version} が利用可能です', 'v{version} 사용 가능', 'Phiên bản v{version} đã có sẵn'),
  ...
  updateCommandHint: _('如果 git pull 失败，可先执行 <code style="background:var(--bg-tertiary);padding:2px 6px;border-radius:4px">git checkout -- .</code> 丢弃本地修改。<br>路径请替换为实际的产品安装目录。', 'If <code style="background:var(--bg-tertiary);padding:2px 6px;border-radius:4px">git pull</code> fails, run <code style="background:var(--bg-tertiary);padding:2px 6px;border-radius:4px">git checkout -- .</code> first to discard local changes.<br>Replace the path with your actual product install directory.', '如果 git pull 失敗，可先執行 <code style="background:var(--bg-tertiary);padding:2px 6px;border-radius:4px">git checkout -- .</code> 捨棄本地修改。<br>路徑請替換為實際的產品安裝目錄。', 'git pull に失敗した場合は、先に <code style="background:var(--bg-tertiary);padding:2px 6px;border-radius:4px">git checkout -- .</code> を実行してローカル変更を破棄してください。<br>パスは実際の製品インストールディレクトリに置き換えてください。', 'git pull 이 실패하면 먼저 <code style="background:var(--bg-tertiary);padding:2px 6px;border-radius:4px">git checkout -- .</code> 를 실행해 로컬 변경을 버리세요.<br>경로는 실제 제품 설치 디렉터리로 바꿔 주세요.', 'Nếu git pull thất bại, hãy chạy <code style="background:var(--bg-tertiary);padding:2px 6px;border-radius:4px">git checkout -- .</code> trước để bỏ các thay đổi cục bộ.<br>Hãy thay đường dẫn bằng thư mục cài đặt sản phẩm thực tế của bạn.'),
  goToWebsite: _('前往智爪工具入口', 'Go to Zhizhua Tool Portal', '前往智爪工具入口'),
  downloadFromWebsite: _('工具入口下载', 'Tool Portal Download', '工具入口下載'),
  ...
  projectOpenClawZhName: _('OpenClaw 中文翻译项目', 'OpenClaw Chinese Translation Project', 'OpenClaw 中文翻譯項目'),
  projectOpenClawZh: _('相关开源汉化项目，提供 OpenClaw 中文界面与国内镜像优化，可作为参考资料了解', 'Related open-source localization project for OpenClaw, providing Chinese UI and domestic mirror optimizations for reference', '相關開源漢化項目，提供 OpenClaw 中文界面與國內鏡像最佳化，可作為參考資料了解'),
  projectClawPanel: _('智爪多智能体工作台产品基座，Tauri v2 桌面应用', 'Product workbench base built with Tauri v2', '智爪多智能體工作台產品基座，Tauri v2 桌面應用'),
  ...
  linkWebsite: _('智爪工具入口', 'Zhizhua Tool Portal', '智爪工具入口'),
  linkMainWebsite: _('AI作品主站', 'AI作品 Main Site', 'AI作品主站'),
  linkOpenClawZh: _('OpenClaw 中文翻译项目', 'OpenClaw Chinese Translation Project', 'OpenClaw 中文翻譯項目'),
  ...
  contributeDesc: _('产品基座基于开源项目定制，欢迎反馈问题与改进建议。', 'This product base is customized from open-source projects. Feedback and improvement suggestions are welcome.', '產品基座基於開源項目定制，歡迎反饋問題與改進建議。', 'この製品基盤はオープンソースプロジェクトをベースにカスタマイズされています。問題報告や改善提案を歓迎します。', '이 제품 베이스는 오픈소스 프로젝트를 기반으로 커스터마이즈되었습니다. 문제 제보와 개선 제안을 환영합니다.'),
  ...
  domesticMirrorHint: _('如需反馈问题或获取商业支持，请通过智爪工具入口联系我们。', 'For feedback or commercial support, contact us through the Zhizhua tool portal.', '如需反饋問題或取得商業支援，請透過智爪工具入口聯絡我們。'),
  companyName: _('AI作品', 'AI作品', 'AI作品'),
  officialWebsite: _('AI作品主站', 'AI作品 Main Site', 'AI作品主站', '公式サイト', '공식 사이트', 'Trang web chính thức', 'Sitio oficial', 'Site oficial', 'Официальный сайт', 'Site officiel', 'Offizielle Website'),
  productWebsite: _('智爪工具入口', 'Zhizhua Tool Portal', '智爪工具入口', 'ツールポータル'),
  openSourceRepo: _('产品支持入口', 'Product Support', '產品支援入口', 'サポート入口', '지원入口', 'Hỗ trợ sản phẩm', 'Soporte del producto', 'Suporte do produto', 'Поддержка продукта', 'Support produit', 'Produktsupport'),
  businessCoop: _('商务与定制', 'Business & Customization', '商務與定制', 'ビジネス連携'),
  contactViaWebsite: _('通过智爪工具入口联系我们', 'Contact us through the Zhizhua tool portal', '通過智爪工具入口聯絡我們', 'ツールポータルからお問い合わせください'),
  companyDesc: _('智爪是 AI作品旗下的多智能体工作台分支，面向最终用户提供 OpenClaw 便携化、汉化增强、模型额度接入、Agent 预设、技能包和企业定制能力。', 'Zhizhua is a multi-agent workbench branch under AI作品, providing portable OpenClaw runtime, localization enhancements, model access, agent presets, skill packs, and enterprise customization.', '智爪是 AI作品旗下的多智能體工作台分支，面向最終用戶提供 OpenClaw 便攜化、漢化增強、模型額度接入、Agent 預設、技能包和企業定制能力。'),
