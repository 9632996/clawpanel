...
export default {
  ...
  choiceSubtitle: _('当前产品支持 OpenClaw 与 Hermes Agent。你可以先安装其中一个，也可以稍后再配置。', 'This product supports OpenClaw and Hermes Agent. Install one now, enable both, or configure later.', '目前產品支援 OpenClaw 與 Hermes Agent。你可以先安裝其中一個，也可以稍後再配置。'),
  ...
  gatewayDesc: _('启动 HTTP API 服务以连接当前工作台', 'Start the HTTP API service to connect with the workbench', '啟動 HTTP API 服務以連接目前工作台'),
  ...
  dashNativePanelDesc: _('原生 Hermes Dashboard（默认 9119，可由智爪平台启动）', 'Native Hermes Dashboard (default 9119; can be started by Zhizhua Platform).', '原生 Hermes Dashboard（預設 9119，可由智爪平台啟動）'),
  dashNativePanelStart: _('启动 Dashboard', 'Start Dashboard', '啟動 Dashboard'),
  dashNativePanelStop: _('停止 Dashboard', 'Stop Dashboard', '停止 Dashboard'),
  ...
  dashNativePanelWindowsDesc: _('当前环境暂不支持直接启动 Hermes Dashboard。请继续使用内置 Hermes 面板，或在兼容环境中运行。',
    'This environment cannot start Hermes Dashboard directly. Continue using the built-in Hermes panel, or run it in a compatible environment.',
    '目前環境暫不支援直接啟動 Hermes Dashboard。請繼續使用內建 Hermes 面板，或在相容環境中執行。',
    '現在の環境では Hermes Dashboard を直接起動できません。内蔵 Hermes パネルを使うか、互換環境で実行してください。',
    '현재 환경에서는 Hermes Dashboard를 직접 시작할 수 없습니다. 내장 Hermes 패널을 계속 사용하거나 호환 환경에서 실행하세요.'),
  dashNativePanelWindowsAlt1: _('继续使用内置的 Hermes 面板（推荐，体验更好）',
    'Keep using the built-in Hermes panel (recommended, better experience)',
    '繼續使用內建的 Hermes 面板（推薦，體驗更好）',
    '内蔵の Hermes パネルを使い続ける（推奨、より良い体験）',
    '내장 Hermes 패널을 계속 사용 (권장, 더 나은 경험)'),
  ...
  configSavedGatewayRestarted: _('配置已保存，Gateway 已重启，新 API Key 已生效', 'Config saved. Gateway restarted and the new API key is active.', '配置已儲存，Gateway 已重新啟動，新 API Key 已生效'),
  configSavedGatewayRestartFailed: _('配置已保存，但 Gateway 重启失败，请手动重启 Gateway', 'Config saved, but Gateway restart failed. Restart Gateway manually.', '配置已儲存，但 Gateway 重新啟動失敗，請手動重新啟動 Gateway'),
  ...
  hermesGatewayForeign: _('端口已被外部进程占用（非当前应用拉起）', 'Port owned by an external process (not spawned by this app)', '埠號已被外部行程佔用'),
  hermesGatewayForeignTip: _('只能停止由当前应用拉起的实例', 'Can only stop instances spawned by this app', '只能停止由目前應用拉起的實例'),
