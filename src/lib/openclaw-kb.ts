...
## 十二、AI作品模型服务
AI作品模型服务为智爪平台用户提供 OpenAI 兼容模型调用能力。产品可内置默认授权，用户也可以在模型配置中改用自己的服务商或 API Key。

### 核心信息
- **API 网关地址**：https://ai.iazp.cn/v1
- **支持接口**：/v1/chat/completions、/v1/responses（OpenAI 兼容）
- **支持模型**：多种顶级 AI 模型（具体可用模型以平台实时列表为准）
- **授权方式**：使用内置默认授权，或在智爪工具入口补充授权，也可切换为用户自己的服务商 API Key
- **用户入口**：https://ai.iazp.cn（查看授权、额度和工具服务）
...
### 获取密钥
1. 优先使用产品内置的默认模型配置
2. 如需测试、补充额度或查看授权状态，前往 https://ai.iazp.cn 处理
3. 高级用户也可以在模型配置中填写自己的 API Key

### 接入方式
已兼容 OpenAI API 的项目，只需替换：
1. Base URL → https://ai.iazp.cn/v1
2. API Key → 内置授权、智爪工具入口授权或用户自己的服务商密钥
即可完成接入。
