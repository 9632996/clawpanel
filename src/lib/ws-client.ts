...
import { KERNEL_TARGET } from './feature-catalog.ts'
...
export class WsClient {
  ...
  connect(host, token, opts = {}) {
    ...
    // 本地 Gateway 是普通 WS；HTTPS 页面只在远程/反代场景默认使用 WSS。
    const secure = opts.secure ?? (!isLocalHost && typeof location !== 'undefined' && location.protocol === 'https:')
