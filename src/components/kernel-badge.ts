 ...
 * 用法：
 *   import { mountKernelBadge } from './components/kernel-badge.ts'
 *   const el = mountKernelBadge(container, { compact: true })
 *
 * 自动订阅 onKernelChange，状态变化时刷新；container 销毁时调用 el._unsub() 清理。
 */
import { onKernelChange, getKernelSnapshot } from '../lib/kernel.ts'
import { FEATURE_CATALOG } from '../lib/feature-catalog.ts'
